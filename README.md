# 🔐 Prodigy InfoTech — Cybersecurity Internship

**Intern:** Olayemi Samson  
**Internship:** Cybersecurity Intern @ [Prodigy InfoTech](https://prodigyinfotech.dev)  
**Duration:** April - May 2026  
**Environment:** Kali Linux (UTM Virtual Machine) | Python 3

---

## 📋 Overview

This repository contains all five tasks completed during my Cybersecurity Internship at Prodigy InfoTech. Each task involved building a working security tool from scratch using Python, covering core concepts in cryptography, image encryption, password security, keylogging, and network analysis.

---

## 🗂️ Repository Structure

```
Prodigy-InfoTech-Cybersecurity-Internship/
│
├── task1-caesar-cipher/
│   ├── caesar_cipher.py
│   └── README.md
│
├── task2-image-cipher/
│   ├── image_cipher_tool.py
│   └── README.md
│
├── task3-password-checker/
│   ├── password_checker.py
│   └── README.md
│
├── task4-keylogger/
│   ├── keylogger.py
│   └── README.md
│
├── task5-packet-analyzer/
│   ├── packet_analyzer.py
│   └── README.md
│
└── README.md
```

---

## 🧰 Tasks Summary

| Task | Title | Key Concept |
|------|-------|-------------|
| Task 1 | Caesar Cipher Tool | Classical cryptography & substitution ciphers |
| Task 2 | Image Encryption Tool | XOR pixel manipulation & symmetric encryption |
| Task 3 | Password Strength Assessor | Password complexity analysis & security feedback |
| Task 4 | Keylogger | Keystroke capture & endpoint security awareness |
| Task 5 | Network Packet Analyzer (NetSniff) | Live traffic capture, protocol analysis & network monitoring |

---

## ⚙️ Requirements

- Python 3.x
- Kali Linux (recommended) or any Linux distro
- Libraries vary per task — see individual task READMEs

---

## ⚠️ Disclaimer

All tools in this repository were built strictly for **educational and ethical purposes** as part of a supervised cybersecurity internship. They must only be used on systems and networks you own or have explicit permission to test. Unauthorized use of these tools is illegal and unethical.

---

## 📬 Contact

**Olayemi Samson**  
Cybersecurity Intern @ Prodigy InfoTech
